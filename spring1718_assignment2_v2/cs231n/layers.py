...
def affine_forward(x, w, b):
    ...
    ###########################################################################
    new_dim = x.shape[0]
    x_reshaped = x.reshape(new_dim, -1)
...
def affine_backward(dout, cache):
    ...
    db = np.sum(dout, axis=0)  # Over columns.
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx, dw, db
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        #######################################################################
        mu = np.mean(x, axis=0)
        var = float(1 / N) * np.sum((x - mu) ** 2, axis=0)
        ...
        for f in range(F):
            for i in range(H_prime):
                for j in range(W_prime):
                    ...
                    # One single value is generated.
                    conv = np.sum(x_with_pad[n, :, patch_h_begin:patch_h_end, patch_w_begin:patch_w_end] *
                                  w[f, :, :, :]) + b[f]
                    out[n, f, i, j] = conv
    ###########################################################################
    #                             END OF YOUR CODE                            #
